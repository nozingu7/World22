#include "..\Public\CGraphic_Device.h"

IMPLEMENT_SINGLETON(CGraphic_Device)

CGraphic_Device::CGraphic_Device()
	: m_p3D(nullptr), m_pDevice(nullptr), m_pFont(nullptr), m_pLine(nullptr), m_pSprite(nullptr)
{

}

HRESULT CGraphic_Device::InitDevice(const GRAPHICDESC& GraphicDesc, LPDIRECT3DDEVICE9& _pOut)
{
...
{
	d3dpp.BackBufferWidth = m_GraphicDesc.iWinCX;
	d3dpp.BackBufferHeight = m_GraphicDesc.iWinCY;
	...
	d3dpp.Windowed = m_GraphicDesc.isWindowed; // TRUE인 경우 창모드, FALSE 전체 화면 모드
...
void CGraphic_Device::Render_Line(const _VECTOR3& _Start, const _VECTOR3& _End, const _float& _fLineWidth, const D3DXCOLOR& _Color)
{
	_float4x4		ViewMatrix, ProjMatrix;
	m_pDevice->GetTransform(D3DTS_VIEW, &ViewMatrix);
	m_pDevice->GetTransform(D3DTS_PROJECTION, &ProjMatrix);

	_VECTOR3 Vertices[2] = { _Start, _End };
	for (_uint i = 0; i < 2; ++i)
	{
		D3DXVec3TransformCoord(&Vertices[i], &Vertices[i], &ViewMatrix);
		D3DXVec3TransformCoord(&Vertices[i], &Vertices[i], &ProjMatrix);
	}
	m_pLine->SetWidth(_fLineWidth);
	_float4x4		Tmp;
	D3DXMatrixIdentity(&Tmp);
	m_pDevice->EndScene();
	m_pDevice->BeginScene();
	m_pLine->Begin();
	m_pLine->DrawTransform(Vertices, 2, &Tmp, _Color);
	m_pLine->End();
	m_pDevice->EndScene();
	m_pDevice->BeginScene();
}

void CGraphic_Device::Render_Lines(void * _pPosList, const _float & _fLineWidth, const D3DXCOLOR & _Color)
{
	_float4x4		ViewMatrix, ProjMatrix;
	m_pDevice->GetTransform(D3DTS_VIEW, &ViewMatrix);
	m_pDevice->GetTransform(D3DTS_PROJECTION, &ProjMatrix);

	vector<_VECTOR3> m_vecPosList;
	memcpy(&m_vecPosList, _pPosList, sizeof(vector <_VECTOR3>));
	if (0 >= m_vecPosList.size())
		return;
	_VECTOR3* Vertices = new _VECTOR3[m_vecPosList.size()];
	for (_uint i = 0; i < m_vecPosList.size(); ++i)
		Vertices[i] = m_vecPosList[i];
	for (_uint i = 0; i < m_vecPosList.size(); ++i)
	{
		D3DXVec3TransformCoord(&Vertices[i], &Vertices[i], &ViewMatrix);
		D3DXVec3TransformCoord(&Vertices[i], &Vertices[i], &ProjMatrix);
	...
	m_pDevice->BeginScene();
	m_pLine->Begin();
	m_pLine->DrawTransform(Vertices, (DWORD)m_vecPosList.size(), &Tmp, _Color);
	m_pLine->End();
	m_pDevice->EndScene();
	m_pDevice->BeginScene();
}
