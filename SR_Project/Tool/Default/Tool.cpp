                     ...
                     _In_ int       nCmdShow)
{
    ...
    MSG msg;
	_bool done = true;
	CMainTool* pMainTool = CMainTool::Create(done);
	...
	_float fDefaultTimerAcc = 0.f;
	const _float SECOND_FRAME = 1.f / 60.f;

	const _tchar* szDefualtTimerName = TEXT("Default_Timer");
	const _tchar* szFrameTimerName = TEXT("Frame_Timer");

	pGameInstance->Add_Timer(szDefualtTimerName);
	pGameInstance->Add_Timer(szFrameTimerName);
    // 기본 메시지 루프입니다:
    while (done)
    {
		...
		fDefaultTimerAcc += pGameInstance->Get_TimeDelta(szDefualtTimerName);
		if (fDefaultTimerAcc >= SECOND_FRAME)
		{
			pMainTool->Tick(pGameInstance->Get_TimeDelta(szFrameTimerName));
			pMainTool->Render();
			fDefaultTimerAcc = 0.f;
		}
    }
